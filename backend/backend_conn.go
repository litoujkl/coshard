...
func (c *Conn) writeAuthHandshake() error {
	...
	if mysql.MYSQL_NATIVE_PASSWORD_AUTH_NAME == c.authPlugin {
		...
		length += 1 + len(auth)
	} else if mysql.CACHING_SHA2_PASSWORD_AUTH_NAME == c.authPlugin {
		auth = mysql.CalcCachingSha2Password(c.salt, []byte(c.password))
		length += 1 + len(auth)
	}
	...
	length += 1 + len(mysql.MYSQL_NATIVE_PASSWORD_AUTH_NAME)
	...
	if mysql.MYSQL_NATIVE_PASSWORD_AUTH_NAME == c.authPlugin {
		pos += 1 + copy(data[pos:], mysql.MYSQL_NATIVE_PASSWORD_AUTH_NAME)
	} else if mysql.CACHING_SHA2_PASSWORD_AUTH_NAME == c.authPlugin {
		pos += 1 + copy(data[pos:], mysql.CACHING_SHA2_PASSWORD_AUTH_NAME)
	}
